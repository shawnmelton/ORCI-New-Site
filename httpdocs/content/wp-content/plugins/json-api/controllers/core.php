...
class JSON_API_Core_Controller {
  ...
  /**
   * Custom functions
   * @author Shawn Melton <shawnmelton@openroadsconsulting.com>
   */

  /**
   * Retrieve the unordered list that is the primary navigation menu for the site.
   * @return array
   */
  public function get_primary_menu() {
      ...
      // Remove /content from urls in order to prevent Wordpress links from getting into site.
      return array(
        'content' => str_replace('/content', '', preg_replace("/(\n|\r|\t)/", '', preg_replace('/ (id|class)=\"[a-z,\-,0-9, ]+\"/i', '', $out)))
      );
  }


  /**
   * Get all images with the category of "Banner" uploaded to the Media Library
   */
